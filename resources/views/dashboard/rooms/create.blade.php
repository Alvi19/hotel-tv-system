...
<div class="p-6 space-y-6">
    ...
    <div class="flex justify-between items-center">
        <h3 class="text-2xl font-semibold text-white flex items-center gap-2">
            <i class="bi bi-door-open text-info"></i> Add New Room
        </h3>
                    ...
                    <div class="form-control w-full">
                        ...
                        <select name="hotel_id" class="select select-bordered w-full bg-base-200 text-white"
                            ...
                            @foreach ($hotels as $hotel)
                                <option value="{{ $hotel->id }}" {{ (int)old('hotel_id') === $hotel->id ? 'selected' : '' }}>
                                    {{ $hotel->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-control w-full">
                    ...
                    <select name="category_id" class="select select-bordered w-full bg-base-200 text-white">
                        ...
                        @foreach ($categories as $cat)
                            <option value="{{ $cat->id }}" {{ (int)old('category_id') === $cat->id ? 'selected' : '' }}>
                                {{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
